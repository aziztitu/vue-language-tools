...
import type { SFCParseResult } from "@azee/vue-language-server";
import { commands } from "@azee/vue-language-server/lib/types";
import {
  executeCommand,
  extensionContext,
  useActiveTextEditor,
...
export async function activate(client: BaseLanguageClient) {
  ...
  async function getProblems(fileUri: vscode.Uri) {
    ...
    const pugPluginPkg = await getPackageJsonOfWorkspacePackage(
      fileUri.fsPath,
      "@azee/vue-language-plugin-pug",
    );

    // check using pug but don't install @azee/vue-language-plugin-pug
    if (sfc?.descriptor.template?.lang === "pug" && !pugPluginPkg) {
      problems.push({
        title: "`@azee/vue-language-plugin-pug` missing",
        message: [
          'For `<template lang="pug">`, the `@azee/vue-language-plugin-pug` plugin is required. Install it using `$ npm install -D @azee/vue-language-plugin-pug` and add it to `vueCompilerOptions.plugins` to support TypeScript intellisense in Pug templates.',
          "",
          "- package.json",
          "```json",
          JSON.stringify(
            { devDependencies: { "@azee/vue-language-plugin-pug": "latest" } },
            undefined,
            2,
          ),
          "```",
    ...
    // check using pug but outdated @azee/vue-language-plugin-pug
    if (
      ...
      problems.push({
        title: "Outdated `@azee/vue-language-plugin-pug`",
        message: [
          "The version of `@azee/vue-language-plugin-pug` is too low, it is required to upgrade to `2.0.5` or later.",
          "",
          "- @azee/vue-language-plugin-pug: " + pugPluginPkg.path,
        ].join("\n"),
      });
    }
