  ...
  for (const intputFile of options.rootNames) {
    ...
    it(`Input: ${shortenPath(intputFile)}, Output: ${shortenPath(expectedOutputFile)}`, () => {
      ...
      program.emit(
        sourceFile,
        (outputFile, text) => {
          expect(outputFile.replace(windowsPathReg, "/")).toBe(
            expectedOutputFile.replace(windowsPathReg, "/"),
          );
          outputText = text;
        },
        undefined,
      ...
      expect(
        outputText ? normalizeNewline(outputText) : undefined,
      ).toMatchSnapshot();
    });
  }
});
