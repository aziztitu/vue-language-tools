...
import { hyphenateAttr, VueVirtualCode } from "@azee/vue-language-core";
    ...
    create(context) {
      ...
      return {
        async provideAutoInsertSnippet(document, selection, change) {
          ...
          const { sfc } = root;
          const blocks = [sfc.script, sfc.scriptSetup].filter(
            (block) => !!block,
          );
          if (!blocks.length) {
            return;
          ...
          for (const [, map] of context.language.maps.forEach(virtualCode)) {
            for (const [sourceOffset] of map.toSourceLocation(
              sourceCodeOffset,
            )) {
          ...
          for (const { ast, startTagEnd, endTagStart } of blocks) {
            if (
              sourceCodeOffset < startTagEnd ||
              sourceCodeOffset > endTagStart
            ) {
              continue;
            }
            if (
              isBlacklistNode(ts, ast, sourceCodeOffset - startTagEnd, false)
            ) {
              return;
            }
          }
          ...
          const props =
            (await tsPluginClient?.getPropertiesAtLocation(
              root.fileName,
              sourceCodeOffset,
            )) ?? [];
...
function isCharacterTyping(
  document: TextDocument,
  change: { text: string; rangeOffset: number; rangeLength: number },
) {
...
function isBlacklistNode(
  ts: typeof import("typescript"),
  node: ts.Node,
  pos: number,
  allowAccessDotValue: boolean,
) {
  if (
    ts.isVariableDeclaration(node) &&
    pos >= node.name.getFullStart() &&
