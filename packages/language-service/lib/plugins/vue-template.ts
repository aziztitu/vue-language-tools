...
import { getComponentSpans } from "@azee/vue-typescript-plugin/lib/common";
...
type InternalItemId = "componentEvent" | "componentProp" | "specialTag";
...
export function create(
  ...
  ) => typeof import("@azee/vue-typescript-plugin/lib/client") | undefined,
  ...
  const baseService =
    mode === "pug"
      ? createPugService({
          getCustomData() {
            return [...customData, ...extraCustomData];
          },
  ...
  return {
    ...
    create(context) {
      ...
      const vBind = builtInData.globalAttributes?.find(
        (x) => x.name === "v-bind",
      );

      if (vOn) {
        const markdown =
          (typeof vOn.description === "string"
            ? vOn.description
            : vOn.description?.value) ?? "";
        const modifiers = markdown.split("\n- ")[4].split("\n").slice(2, -1);
      ...
      if (vBind) {
        const markdown =
          (typeof vBind.description === "string"
            ? vBind.description
      ...
      const disposable = context.env.onDidChangeConfiguration?.(
      ...
      return {
        ...
        async provideCompletionItems(
          ...
          if (sourceScript?.generated) {
            const virtualCode =
              sourceScript.generated.embeddedCodes.get("template");
            if (virtualCode) {
              const embeddedDocumentUri = context.encodeEmbeddedDocumentUri(
                sourceScript.id,
                virtualCode.id,
              );
              afterHtmlCompletion(
        ...
        async provideInlayHints(document) {
          ...
          const components =
            (await tsPluginClient?.getComponentNames(root.fileName)) ?? [];
          ...
          while ((token = scanner.scan()) !== html.TokenType.EOS) {
            if (token === html.TokenType.StartTag) {
              ...
              const checkTag = tagName.includes(".")
                ...
                : components.find(
                    (component) =>
                      component === tagName ||
                      hyphenateTag(component) === tagName,
                  );
              if (checkTag) {
                componentProps[checkTag] ??= (
                  (await tsPluginClient?.getComponentProps(
                    root.fileName,
                    checkTag,
                  )) ?? []
                )
              ...
              if (current) {
                ...
                if (attrText === "v-bind") {
                  ...
                  if (attrText.startsWith("v-bind:")) {
                    ...
                    attrText =
                      vueCompilerOptions.target >= 3 ? "modelValue" : "value"; // TODO: support for experimentalModelPropName?
                  } else if (attrText.startsWith("v-on:")) {
                    attrText =
                      "on-" + hyphenateAttr(attrText.slice("v-on:".length));
                  } else if (attrText.startsWith("@")) {
                    attrText =
                      "on-" + hyphenateAttr(attrText.slice("@".length));
                  }

                  current.unburnedRequiredProps =
                    current.unburnedRequiredProps.filter((propName) => {
                      return (
                        attrText !== propName &&
              ...
              token === html.TokenType.StartTagSelfClose ||
              token === html.TokenType.StartTagClose
            ) {
              if (current) {
            ...
            if (
              token === html.TokenType.AttributeName ||
              token === html.TokenType.AttributeValue
            ) {
              if (current) {
                current.insertOffset =
                  scanner.getTokenOffset() + scanner.getTokenLength();
              }
            }
          }
        ...
        async provideDiagnostics(document, token) {
          ...
          const originalResult = await baseServiceInstance.provideDiagnostics?.(
            document,
            token,
          );
          ...
          return [...(originalResult ?? []), ...templateErrors];
        },
        ...
        provideDocumentSemanticTokens(document, range, legend) {
          ...
          const languageService = context.inject<
            import("volar-service-typescript").Provide,
            ...
            provideTags: () => {
              if (!components) {
                promises.push(
                  (async () => {
                    components = (
                      (await tsPluginClient?.getComponentNames(
                        vueCode.fileName,
                      )) ?? []
                    ).filter(
                      (name) =>
              ...
              const scriptSetupRanges = tsCodegen
                .get(vueCode.sfc)
              ...
              for (const binding of scriptSetupRanges?.bindings ?? []) {
                const name = vueCode.sfc.scriptSetup!.content.slice(
                  binding.range.start,
                  binding.range.end,
                );
                if (casing.tag === TagNameCasing.Kebab) {
            ...
            provideAttributes: (tag) => {
              ...
              if (!tagInfo) {
                promises.push(
                  (async () => {
                    const attrs =
                      (await tsPluginClient?.getElementAttrs(
                        vueCode.fileName,
                        tag,
                      )) ?? [];
                    const propInfos =
                    ...
                    const directives =
                      (await tsPluginClient?.getComponentDirectives(
                        vueCode.fileName,
                      )) ?? [];
                    tagInfos.set(tag, {
                      attrs,
                      propInfos: propInfos.filter(
                        (prop) => !prop.name.startsWith("ref_"),
                      ),
                      events,
                      directives,
                    });
              ...
              for (const prop of [...props, ...attrs]) {
                ...
                const name =
                  casing.attr === AttrNameCasing.Camel
                    ? prop
                    : hyphenateAttr(prop);
                ...
                if (isEvent) {
                  const propNameBase = name.startsWith("on-")
                    ...
                    : name["on".length].toLowerCase() +
                      name.slice("onX".length);
      ...
      function afterHtmlCompletion(
            ...
            for (const modifier of modelData.globalAttributes ?? []) {
              ...
              const newItem: html.CompletionItem = {
                ...
                documentation: {
                  kind: "markdown",
                  value:
                    (typeof modifier.description === "object"
        ...
        completionList.items = completionList.items.filter(
          (item) => !specialTags.has(parseLabel(item.label).name),
        );
        ...
        for (const item of completionList.items) {
          ...
          if (parsedItemKey) {
            ...
            if (propInfo?.commentMarkdown) {
              ...
              item.documentation = {
                kind: "markdown",
                value: [propInfo.commentMarkdown, originalDocumentation]
                  .filter((str) => !!str)
                  .join("\n\n"),
              };
            }
          ...
          if (propInfo?.deprecated) {
            item.tags = [
              1 satisfies typeof vscode.CompletionItemTag.Deprecated,
            ];
          }
          ...
          if (
            item.kind ===
              (10 satisfies typeof vscode.CompletionItemKind.Property) &&
  ...
  function getScanner(
    ...
    if (mode === "html") {
      return service.provide["html/languageService"]().createScanner(
        document.getText(),
      );
    } else {
      const pugDocument = service.provide["pug/pugDocument"](document);
      if (pugDocument) {
        return service.provide["pug/languageService"]().createScanner(
          pugDocument,
        );
      }
    }
...
function getPropName(itemKey: ReturnType<typeof parseItemKey> & {}) {
