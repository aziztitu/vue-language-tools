...
export function create(): LanguageServicePlugin {
  const htmlService = createHtmlService({
    ...
    getCustomData(context) {
      sfcDataProvider ??= html.newHTMLDataProvider(
        "vue",
        loadLanguageBlocks(context.env.locale ?? "en"),
      );
      ...
      return {
        ...
        async provideCompletionItems(document, position, context, token) {
          ...
          result.items = result.items.filter(
            (item) =>
              item.label !== "!DOCTYPE" && item.label !== "Custom Blocks",
          );
  ...
  function worker<T>(
    ...
    callback: (root: VueVirtualCode) => T,
  ) {
