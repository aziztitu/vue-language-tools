import { VueVirtualCode, forEachEmbeddedCode } from "@azee/vue-language-core";
    ...
    create(context) {
      ...
      return {
        async provideDocumentDropEdits(document, _position, dataTransfer) {
          ...
          const code = [...forEachEmbeddedCode(root)].find(
            (code) =>
              code.id === (sfc.scriptSetup ? "scriptsetup_raw" : "script_raw"),
          )!;
          const lastImportNode = getLastImportNode(ts, script.ast);
          const incomingFileName =
            context.project.typescript?.uriConverter.asFileName(
              URI.parse(importUri),
          ...
          if (tsPluginClient && serviceScript) {
            const tsDocumentUri = context.encodeEmbeddedDocumentUri(
              sourceScript.id,
              serviceScript.code.id,
            );
          ...
          if (!importPath) {
            importPath =
              path.relative(path.dirname(root.fileName), incomingFileName) ||
          ...
          const embeddedDocumentUriStr = context
            .encodeEmbeddedDocumentUri(sourceScript.id, code.id)
            .toString();
          ...
          additionalEdit.changes[embeddedDocumentUriStr].push({
            range: lastImportNode
              ? {
                  start: script.ast.getLineAndCharacterOfPosition(
                    lastImportNode.end,
                  ),
                  end: script.ast.getLineAndCharacterOfPosition(
                    lastImportNode.end,
                  ),
                }
              : {
