import { tsCodegen, VueVirtualCode } from "@azee/vue-language-core";
import { collectIdentifiers } from "@azee/vue-language-core/lib/codegen/utils/index";
      ...
      return {
        async provideInlayHints(document, range) {
          ...
          async function getSettingEnabled(key: string) {
            return (settings[key] ??=
              (await context.env.getConfiguration?.<boolean>(key)) ?? false);
          }
          ...
          for (const hint of inlayHints) {
            const block = blocks.find(
              (block) => block?.name === hint.blockName,
            );
            if (!block) {
              continue;
            }
...
export function findDestructuredProps(
  ...
  function walkScope(node: ts.Node, isRoot = false) {
    ts.forEachChild(node, (stmt) => {
      if (ts.isVariableStatement(stmt)) {
        ...
        const declare = ts
          .getModifiers(stmt)
          ?.find((modifier) => modifier.kind === ts.SyntaxKind.DeclareKeyword);
  ...
  function isReferencedIdentifier(id: ts.Identifier, parent: ts.Node | null) {
    if (!parent) {
      return false;
    }
