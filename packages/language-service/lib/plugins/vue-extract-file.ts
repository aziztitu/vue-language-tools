...
import { Sfc, VueVirtualCode, tsCodegen } from "@azee/vue-language-core";
    ...
    create(context) {
      ...
      return {
        provideCodeActions(document, range, ctx) {
          ...
          const templateCodeRange = selectTemplateCode(
            startOffset,
            endOffset,
            sfc.template,
          );
        ...
        async resolveCodeAction(codeAction) {
          ...
          const toExtract =
            (await tsPluginClient?.collectExtractProps(
              root.fileName,
              templateCodeRange,
            )) ?? [];
          if (!toExtract) {
          ...
          const newUri =
            sfcDocument.uri.slice(0, sfcDocument.uri.lastIndexOf("/") + 1) +
            `${newName}.vue`;
              ...
              range: lastImportNode
                ? {
                    start: sfcDocument.positionAt(
                      script.startTagEnd + lastImportNode.end,
                    ),
          ...
          if (sfc.script) {
            const edit = createAddComponentToOptionEdit(
              ts,
              sfc,
              sfc.script.ast,
              newName,
            ...
            if (edit) {
              sfcEdits.push({
                range: {
                  start: sfcDocument.positionAt(
                    sfc.script.startTagEnd + edit.range.start,
                  ),
                  end: sfcDocument.positionAt(
                    sfc.script.startTagEnd + edit.range.end,
                  ),
                },
          ...
          function generateNewScriptContents() {
            ...
            for (const model of models) {
              lines.push(
                `const ${model.name} = defineModel<${model.type}>('${model.name}', { required: true })`,
              );
            }
            return lines.join("\n");
          }
...
function selectTemplateCode(
  ...
  templateBlock.ast?.children.forEach(function visit(
  ...
  if (insideNodes.length) {
    const first = insideNodes.sort(
      (a, b) => a.loc.start.offset - b.loc.start.offset,
    )[0];
    const last = insideNodes.sort(
      (a, b) => b.loc.end.offset - a.loc.end.offset,
    )[0];
...
export function getLastImportNode(
  ts: typeof import("typescript"),
