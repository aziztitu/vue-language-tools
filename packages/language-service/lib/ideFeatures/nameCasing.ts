...
import type { CompilerDOM } from "@azee/vue-language-core";
...
export async function convertTagName(
  ...
  tsPluginClient:
    | typeof import("@azee/vue-typescript-plugin/lib/client")
    | undefined,
) {
...
export async function convertAttrName(
  ...
  tsPluginClient?: typeof import("@azee/vue-typescript-plugin/lib/client"),
) {
  const sourceFile = context.language.scripts.get(uri);
  if (!sourceFile) {
    return;
  ...
  for (const [tagName, { attrs }] of tags) {
    ...
    if (componentName) {
      ...
      for (const [attrName, { offsets }] of attrs) {
        const propName = props.find(
          (prop) => prop === attrName || hyphenateAttr(prop) === attrName,
        );
        if (propName) {
          for (const offset of offsets) {
            const start = document.positionAt(template.startTagEnd + offset);
            const end = document.positionAt(
              template.startTagEnd + offset + attrName.length,
            );
            const range: vscode.Range = { start, end };
            if (
              casing === AttrNameCasing.Kebab &&
              attrName !== hyphenateAttr(propName)
            ) {
...
export async function getNameCasing(context: LanguageServiceContext, uri: URI) {
  ...
  const [attr, tag] = await Promise.all([
    context.env.getConfiguration?.<
      "autoKebab" | "autoCamel" | "kebab" | "camel"
    >("vue.complete.casing.props", uri.toString()),
    context.env.getConfiguration?.<
      "autoKebab" | "autoPascal" | "kebab" | "pascal"
    >("vue.complete.casing.tags", uri.toString()),
  ]);
  const tagNameCasing =
    detected.tag.length === 1 && (tag === "autoPascal" || tag === "autoKebab")
      ? detected.tag[0]
      : tag === "autoKebab" || tag === "kebab"
        ? TagNameCasing.Kebab
        : TagNameCasing.Pascal;
  const attrNameCasing =
    detected.attr.length === 1 && (attr === "autoCamel" || attr === "autoKebab")
      ? detected.attr[0]
...
export async function detect(
  ...
  function getTagNameCase(
    file: VueVirtualCode,
  ): ProviderResult<TagNameCasing[]> {
        ...
        for (const node of vue.forEachElementNode(ast)) {
          ...
          for (const prop of node.props) {
            ...
            if (
              ...
              prop.arg?.type ===
                (4 satisfies CompilerDOM.NodeTypes.SIMPLE_EXPRESSION) &&
              prop.arg.isStatic
            ) {
