...
export function initialize(
  ...
  return server.initialize(
    ...
    createTypeScriptProject(
      ...
      async ({ configFileName, sys, uriConverter }) => {
        ...
        if (configFileName) {
          let commandLine = createParsedCommandLine(
            ts,
            sys,
            configFileName,
            true,
          ...
          while (sysVersion !== newSysVersion) {
            commandLine = createParsedCommandLine(
              ts,
              sys,
              configFileName,
              true,
          ...
          setup({ project }) {
            ...
            if (project.typescript) {
              const directoryExists =
                project.typescript.languageServiceHost.directoryExists?.bind(
                  project.typescript.languageServiceHost,
                );
              const fileExists =
                project.typescript.languageServiceHost.fileExists.bind(
                  project.typescript.languageServiceHost,
                );
              const getScriptSnapshot =
                project.typescript.languageServiceHost.getScriptSnapshot.bind(
                  project.typescript.languageServiceHost,
                );
              const globalTypesName =
                getGlobalTypesFileName(vueCompilerOptions);
              ...
              if (directoryExists) {
                project.typescript.languageServiceHost.directoryExists = (
                  path,
                ) => {
              ...
              project.typescript.languageServiceHost.getScriptSnapshot = (
                path,
              ) => {
                if (
                  path.endsWith(`.vue-global-types/${globalTypesName}`) ||
  ...
  function updateFileWatcher(vueCompilerOptions: VueCompilerOptions) {
    ...
    const newExtensions = extensions.filter(
      (ext) => !watchingExtensions.has(ext),
    );
