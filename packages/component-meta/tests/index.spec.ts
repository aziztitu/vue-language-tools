...
const worker = (checker: ComponentMetaChecker, withTsconfig: boolean) =>
  describe(`@azee/vue-component-meta ${withTsconfig ? "with tsconfig" : "without tsconfig"}`, () => {
    ...
    test("reference-type-model", () => {
      ...
      const onUpdateModelValue = meta.events.find(
        (event) => event.name === "update:modelValue",
      );

      const foo = meta.props.find((prop) => prop.name === "foo");
      const onUpdateFoo = meta.events.find(
        (event) => event.name === "update:foo",
      );

      const bar = meta.props.find((prop) => prop.name === "bar");
      const barModifiers = meta.props.find(
        (prop) => prop.name === "barModifiers",
      );
      const onUpdateBaz = meta.events.find(
        (event) => event.name === "update:bar",
      );
    ...
    test("reference-type-props", () => {
      ...
      const unionOptional = meta.props.find(
        (prop) => prop.name === "unionOptional",
      );
      const nested = meta.props.find((prop) => prop.name === "nested");
      const nestedIntersection = meta.props.find(
        (prop) => prop.name === "nestedIntersection",
      );
      const nestedOptional = meta.props.find(
        (prop) => prop.name === "nestedOptional",
      );
      const array = meta.props.find((prop) => prop.name === "array");
      const arrayOptional = meta.props.find(
        (prop) => prop.name === "arrayOptional",
      );
      const enumValue = meta.props.find((prop) => prop.name === "enumValue");
      const literalFromContext = meta.props.find(
        (prop) => prop.name === "literalFromContext",
      );
      ...
      expect(nestedIntersection?.type).toEqual(
        "MyNestedProps & { additionalProp: string; }",
      );
      expect(nestedIntersection?.description).toEqual(
      ...
      expect(nestedOptional?.type).toEqual(
        "MyNestedProps | MyIgnoredNestedProps | undefined",
      );
      ...
      expect(literalFromContext?.type).toEqual(
        '"Uncategorized" | "Content" | "Interaction" | "Display" | "Forms" | "Addons"',
      );
      expect(literalFromContext?.description).toEqual(
    ...
    test("reference-type-props-js-setup", () => {
      ...
      const numberOrStringProp = meta.props.find(
        (prop) => prop.name === "numberOrStringProp",
      );
    ...
    test("reference-type-events", () => {
      ...
      expect(onFoo?.signature).toEqual(
        '(event: "foo", data?: { foo: string; } | undefined): void',
      );
      ...
      expect(onBar?.signature).toEqual(
    ...
    test("template-slots", () => {
      const componentPath = path.resolve(
        __dirname,
        "../../../test-workspace/component-meta/template-slots/component.vue",
      );
      const meta = checker.getComponentMeta(componentPath);

      expect(meta.type).toEqual(TypeMeta.Class);

      const a = meta.slots.find(
        (slot) => slot.name === "default" && slot.type === "{ num: number; }",
      );
      const b = meta.slots.find(
        (slot) =>
          slot.name === "named-slot" && slot.type === "{ str: string; }",
      );
      const c = meta.slots.find(
        (slot) =>
          slot.name === "vbind" &&
    ...
    test("template-slots for generic", () => {
      ...
      expect(
        meta.slots.find(
          (slot) => slot.name === "default" && slot.type === "{ foo: number; }",
        ),
      ).toBeDefined();
    });
    ...
    test("template-slots without a script block", () => {
      const componentPath = path.resolve(
        __dirname,
        "../../../test-workspace/component-meta/template-slots/component-no-script.vue",
      );
      const meta = checker.getComponentMeta(componentPath);
    ...
    test("class-slots", () => {
      ...
      const b = meta.slots.find(
        (slot) => slot.name === "foo" && slot.type === "{ str: string; }",
      );
    ...
    test("exposed", () => {
      ...
      const counter = meta.exposed.find(
        (exposed) =>
    ...
    test("ts-component", () => {
      ...
      const a = meta.props.find(
        (prop) =>
          prop.name === "foo" &&
          prop.required === true &&
          prop.type === "string",
      );
      const b = meta.props.find(
        (prop) =>
          prop.name === "bar" &&
          prop.required === false &&
    ...
    test("ts-named-exports", () => {
      ...
      const a = Foo.props.find(
        (prop) =>
          prop.name === "foo" &&
          prop.required === true &&
          prop.type === "string",
      );
      const b = Bar.props.find(
        (prop) =>
          prop.name === "bar" &&
          prop.required === false &&
          prop.type === "number | undefined",
    ...
    test("options-api", () => {
      const componentPath = path.resolve(
        __dirname,
        "../../../test-workspace/component-meta/options-api/component.ts",
      );
      const meta = checker.getComponentMeta(componentPath);
      ...
      const propNumberDefault = meta.props.find(
        (prop) => prop.name === "numberDefault",
      );
      ...
      const propObjectDefault = meta.props.find(
        (prop) => prop.name === "objectDefault",
      );

      expect(propObjectDefault).toBeDefined();
      expect(propObjectDefault?.default).toEqual(`{\n    foo: "bar"\n}`);

      const propArrayDefault = meta.props.find(
        (prop) => prop.name === "arrayDefault",
      );
