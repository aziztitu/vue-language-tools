...
import { code as typeHelpersCode } from "@azee/vue-component-type-helpers";
import { code as vue2TypeHelpersCode } from "@azee/vue-component-type-helpers/vue2";
...
export function baseCreate(
  ...
  let fileNames = commandLine.fileNames.map((path) =>
    path.replace(windowsPathReg, "/"),
  );
    ...
    (fileName) => {
      ...
      if (fileName === globalComponentName) {
        ...
        if (!metaSnapshots.has(fileName)) {
          metaSnapshots.set(
            fileName,
            ts.ScriptSnapshot.fromString(getMetaScriptContent(fileName)),
          );
        }
  ...
  const directoryExists =
    languageServiceHost.directoryExists?.bind(languageServiceHost);
  ...
  return {
    ...
    reload() {
      ...
      fileNames = commandLine.fileNames.map((path) =>
        path.replace(windowsPathReg, "/"),
      );
      this.clearCache();
    },
...
`.trim();
  ...
  function getExportNames(componentPath: string) {
    ...
    return _getExports(program, typeChecker, componentPath).exports.map((e) =>
      e.getName(),
    );
  }
  ...
  function getComponentMeta(
    ...
    const program = tsLs.getProgram()!;
    const typeChecker = program.getTypeChecker();
    const { symbolNode, exports } = _getExports(
      program,
      typeChecker,
      componentPath,
    );
    ...
    function getType() {
      const $type = symbolProperties.find(
        (prop) => prop.escapedName === "type",
      );
    ...
    function getProps() {
      ...
      if ($props) {
        ...
        result = properties
          .map((prop) => {
            const { resolveNestedProperties } = createSchemaResolvers(
              typeChecker,
              symbolNode,
              checkerOptions,
              ts,
      ...
      if (componentPath !== globalComponentName) {
        globalPropNames ??= getComponentMeta(globalComponentName).props.map(
          (prop) => prop.name,
        );
      ...
      const vueDefaults =
        vueFile && exportName === "default"
          ? vueFile instanceof vue.VueVirtualCode
            ? readVueComponentDefaultProps(vueFile, printer, ts)
            : {}
          : {};
    ...
    function getEvents() {
      const $emit = symbolProperties.find(
        (prop) => prop.escapedName === "emit",
      );
      ...
      if ($emit) {
        ...
        return calls
          .map((call) => {
            const { resolveEventSignature } = createSchemaResolvers(
              typeChecker,
              symbolNode,
              checkerOptions,
              ts,
    ...
    function getSlots() {
      ...
      if ($slots) {
        ...
        return properties.map((prop) => {
          const { resolveSlotProperties } = createSchemaResolvers(
            typeChecker,
            symbolNode,
            checkerOptions,
            ts,
    ...
    function getExposed() {
      ...
      if ($exposed) {
        ...
        const properties = type.getProperties().filter(
          (prop) =>
        ...
        return properties.map((prop) => {
          const { resolveExposedProperties } = createSchemaResolvers(
            typeChecker,
            symbolNode,
            checkerOptions,
            ts,
...
function createSchemaResolvers(
  ...
  function resolveEventSignature(call: ts.Signature): EventMeta {
    ...
    return {
      ...
      description: ts.displayPartsToString(
        call.getDocumentationComment(typeChecker),
      ),
      ...
      get declarations() {
        return (declarations ??= call.declaration
          ? getDeclarations([call.declaration])
          : []);
      },
      get schema() {
        return (schema ??= typeChecker
...
function readVueComponentDefaultProps(
  ...
  function scriptSetupWorker() {
    ...
    if (scriptSetupRanges?.withDefaults?.argNode) {
      const obj = findObjectLiteralExpression(
        scriptSetupRanges.withDefaults.argNode,
      );
      if (obj) {
      ...
      const obj = findObjectLiteralExpression(
        scriptSetupRanges.defineProps.argNode,
      );
      if (obj) {
        result = {
          ...result,
      ...
      for (const [name, initializer] of scriptSetupRanges.defineProps
        .destructured) {
        if (initializer) {
          const expText =
            printer?.printNode(ts.EmitHint.Expression, initializer, ast) ??
    ...
    if (scriptSetupRanges?.defineProp) {
      for (const defineProp of scriptSetupRanges.defineProp) {
        const obj = defineProp.argNode
          ? findObjectLiteralExpression(defineProp.argNode)
          : undefined;
        if (obj) {
          const name = defineProp.name
            ? sfc.scriptSetup.content
                .slice(defineProp.name.start, defineProp.name.end)
                .slice(1, -1)
            : "modelValue";
