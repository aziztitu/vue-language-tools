import { FileMap } from "@azee/vue-language-core";
...
const pipeDir = platform === "win32" ? `\\\\.\\pipe\\` : `/tmp/`;
...
class NamedPipeServer {
  ...
  containsFile(fileName: string) {
    if (this.projectInfo) {
      if (!this.containsFileCache.has(fileName)) {
        this.containsFileCache.set(
          fileName,
          (async () => {
            const res = await this.sendRequest<boolean>(
              "containsFile",
              fileName,
            );
  ...
  onData(chunk: Buffer) {
    ...
    if (text.endsWith("\n\n")) {
      ...
      for (let result of results) {
        ...
        try {
          const data: ResponseData | NotificationData = JSON.parse(
            result.trim(),
          );
  ...
  onNotification(type: NotificationData[0], fileName: string, data: any) {
    ...
    if (type === "componentNamesUpdated") {
      ...
      const [name, props]: [name: string, props: ComponentPropInfo[]] = data;
...
for (let i = 0; i < 10; i++) {
  configuredServers.push(
    new NamedPipeServer(1 satisfies ts.server.ProjectKind.Configured, i),
  );
  inferredServers.push(
    new NamedPipeServer(0 satisfies ts.server.ProjectKind.Inferred, i),
  );
}
...
export async function getBestServer(fileName: string) {
  ...
  servers.sort((a, b) =>
    sortTSConfigs(fileName, a.projectInfo!.name, b.projectInfo!.name),
  );
  ...
  servers = (
    await Promise.all(
      inferredServers.map((server) => {
        ...
        if (
          path.relative(projectInfo.currentDirectory, fileName).startsWith("..")
        ) {
          return;
        }
        return server;
