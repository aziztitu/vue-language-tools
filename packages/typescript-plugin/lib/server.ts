import { FileMap, Language } from "@azee/vue-language-core";
...
import {
  ...
  getComponentProps,
} from "./requests/getComponentProps";
...
export type ResponseData = [seq: number, data: any];
...
export async function startNamedPipeServer(
  ...
  const currentData = new FileMap<
    [componentNames: string[], Record<string, ComponentPropInfo[]>]
  >(false);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const namedPipeOccupied =
      typeof socket === "object" || socket === "timeout";
  ...
  async function updateWhile() {
    while (true) {
      ...
      const openedScriptInfos = info.project
        .getRootScriptInfos()
        .filter((info) => info.isScriptOpen());
      ...
      for (const scriptInfo of openedScriptInfos) {
        ...
        const newComponentNames =
          getComponentNames.apply(requestContext, [scriptInfo.fileName]) ?? [];

        if (
          JSON.stringify(oldComponentNames) !==
          JSON.stringify(newComponentNames)
        ) {
        ...
        for (const [name, props] of Object.entries(componentProps)) {
          ...
          const newProps =
            getComponentProps.apply(requestContext, [
              scriptInfo.fileName,
              name,
            ]) ?? [];
          if (JSON.stringify(props) !== JSON.stringify(newProps)) {
            ...
            for (const connection of connections) {
              notify(connection, "componentPropsUpdated", scriptInfo.fileName, [
                name,
                newProps,
              ]);
            }
  ...
  function notify(
    ...
    type: NotificationData[0],
    fileName: string,
    data: any,
  ) {
    connection.write(
      JSON.stringify([type, fileName, data] satisfies NotificationData) +
        "\n\n",
    );
  }
  ...
  function onRequest(
    connection: net.Socket,
    [seq, requestType, ...args]: RequestData,
  ) {
  ...
  function handleRequest(
    ...
    if (requestType === "projectInfo") {
      ...
      const tag = args[1];
      const props =
        getComponentProps.apply(requestContext, [fileName, tag]) ?? [];
